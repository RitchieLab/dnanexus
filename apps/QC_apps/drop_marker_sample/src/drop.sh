...
main() {
    ...
    if [ -n "$marker_drop" ]
    then
        dx download "$marker_drop" -o marker_drop
		PLINK_ARGS="$PLINK_ARGS --exclude marker_drop"
    fi
    if [ -n "$sample_drop" ]
    then
        dx download "$sample_drop" -o sample_drop
		PLINK_ARGS="$PLINK_ARGS --remove sample_drop"
    fi
	...
	if test -z "$PLINK_ARGS"; then
		dx-jobutil-report-error "You must supply either a marker drop list or a sample drop list"
	fi
