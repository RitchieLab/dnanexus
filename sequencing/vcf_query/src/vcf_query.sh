...
main() {
	...
	while read VCF_LINE; do
		...
		SUBJOB=$(dx-jobutil-new-job run_anno -ivcf_fn:file="$VCF_DXFN" -ivcfidx_fn:file="$VCFIDX_DXFN" -iquery_str:string=$query_str)
...
run_anno() {
	...
	vcf-query input.vcf.gz $query_str | bgzip -c > $OUTDIR/$PREFIX.query.gz
